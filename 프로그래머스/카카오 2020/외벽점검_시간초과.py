def solution(n, weak, dist):
    def check_visited(weak_idx, dist_value, weak_visited):
        time = 0
            ...
            if not weak_visited[weak_idx]:
                for dist_idx in range(len(dist)):
                    if not dist_selected[dist_idx]:
                        weak_visited, visited_index = check_visited(weak_idx, dist[dist_idx], weak_visited)
    ...
    return check_outside(len(dist), weak_visited)
